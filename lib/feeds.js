...
export function canonicalizeFeed(xml, locale, base, emptyFeedUpdated) {
  const document = new DOMParser().parseFromString(xml, 'text/xml');
  const feed = document.documentElement;
  const empty = !Array.from(feed.children).some(child => child.localName === 'entry');
  for (const child of feed.children) {
    // The plugin defaults to wall-clock time for an empty collection. Keep
    // repeated builds stable, preserving its actual newest date when populated.
    if (empty && child.localName === 'updated') child.textContent = emptyFeedUpdated;
